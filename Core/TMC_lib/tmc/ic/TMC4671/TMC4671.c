...
int32_t tmc4671_readInt(uint8_t motor, uint8_t address)
{
	...
	MOTION_CTRL_CS_LOW();
	MOTION_CTRL_SPI_READ_WRITE((uint8_t*)&address, 		(uint8_t*)&readBuff, sizeof(address)); 		// write address
	MOTION_CTRL_SPI_READ_WRITE((uint8_t*)&writeBuff, 	(uint8_t*)&readBuff, sizeof(readBuff)); 	// read data
	MOTION_CTRL_CS_HIGH();
...
void tmc4671_writeInt(uint8_t motor, uint8_t address, int32_t value)
{
	...
	MOTION_CTRL_CS_LOW();
	MOTION_CTRL_SPI_READ_WRITE((uint8_t*)&address, 	(uint8_t*)&readBuff, sizeof(address)); 		// write address
	MOTION_CTRL_SPI_READ_WRITE((uint8_t*)&value, 	(uint8_t*)&readBuff, sizeof(value)  ); 	// write value
	MOTION_CTRL_CS_HIGH();
//	// write address
