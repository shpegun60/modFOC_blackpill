...
#define TMC4671_SPI_READ_WRITE(tx_ptr, rx_ptr, size) HAL_SPI_TransmitReceive(&hspi1, (tx_ptr), (rx_ptr), (size), HAL_MAX_DELAY)
#define TMC4671_CS_LOW() HAL_GPIO_WritePin(CS_CTRL_GPIO_Port, CS_CTRL_Pin, GPIO_PIN_RESET)
#define TMC4671_CS_HIGH() HAL_GPIO_WritePin(CS_CTRL_GPIO_Port, CS_CTRL_Pin, GPIO_PIN_SET)
...
int32_t tmc4671_readInt(uint8_t motor, uint8_t address)
{
	...
	TMC4671_CS_LOW();
	TMC4671_SPI_READ_WRITE((uint8_t*)&address, 		(uint8_t*)&readBuff, sizeof(address)); 		// write address
	TMC4671_SPI_READ_WRITE((uint8_t*)&writeBuff, 	(uint8_t*)&readBuff, sizeof(readBuff)); 	// read data
	TMC4671_CS_HIGH();
...
void tmc4671_writeInt(uint8_t motor, uint8_t address, int32_t value)
{
	...
	TMC4671_CS_LOW();
	TMC4671_SPI_READ_WRITE((uint8_t*)&address, 	(uint8_t*)&readBuff, sizeof(address)); 		// write address
	TMC4671_SPI_READ_WRITE((uint8_t*)&value, 	(uint8_t*)&readBuff, sizeof(value)  ); 	// write value
	TMC4671_CS_HIGH();
